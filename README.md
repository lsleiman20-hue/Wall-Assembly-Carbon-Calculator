...
## Exact chart values update

- Added exact numeric labels directly to the intensity and total-carbon bars.
- Added readable segment values to the stacked material-breakdown chart when space allows.
- Added exact-value tables below every comparison chart in both the app and the downloadable PDF report, so small stacked segments are still documented precisely.
