...
st.markdown(
    ...
    <style>
      ...
      button, input, textarea, label, p, div, span, h1, h2, h3, h4 {
      ...
      [data-testid="stSidebar"] input { color: #17253A !important; }
      [data-testid="stSidebar"] div[data-baseweb="select"] * { color: #17253A; }
...
if page == "Build Assemblies":
    st.markdown(
        ...
        <div class="hero">
          ...
          <p>Choose one material—or None—for each standard wall layer. Add extra layers only where the assembly needs them.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with st.container(border=True):
        ...
        active["notes"] = st.text_area("Notes / key differences", value=active.get("notes", ""), key=f'notes_{active["id"]}', height=72)

    summary_placeholder = st.empty()

    st.markdown("### Wall layers")
    st.caption("Each category starts with one layer. Select None when the assembly does not use that layer.")
        ...
        with st.container(border=True):
            ...
            with title_col:
                ...
                st.markdown(
                    ...
                    f'<div class="category-copy">{CATEGORY_DESCRIPTIONS[category]}</div>',
                    unsafe_allow_html=True,
                )
            with add_col:
            ...
            for layer_number, material in enumerate(category_materials, start=1):
                ...
                icon = "●" if existing["valid"] else ("○" if not selected else "!")
                expanded = selected and not existing["valid"]

                with st.expander(f'{icon}  Layer {layer_number} — {title}', expanded=expanded):
                    ...
                    if material["preset"] == "None":
                        st.markdown('<div class="layer-empty">This layer is not included in the assembly.</div>', unsafe_allow_html=True)
                        continue
