...
if page == "Build Assemblies":
    ...
    if ready_rows:
        ...
        # Plotly displays the first horizontal-bar category at the bottom. Read
        # the sorted rows in reverse to obtain the visible top-to-bottom bar
        # order, then use that order for the unique category legend entries.
        material_bar_order = chart_df["Material"].tolist()
        contribution_legend_order = list(
            dict.fromkeys(chart_df.iloc[::-1]["Category"].tolist())
        )
        ...
        fig.update_yaxes(categoryorder="array", categoryarray=material_bar_order)
