    ...
    if stacked:
        ...
        for category_index in range(len(category_names)):
            ...
            for _, values, hex_color in series:
                ...
                drawing.add(Rect(x, min(y1, y2), bar_width, max(0.8, segment_height), fillColor=rl_colors.HexColor(hex_color), strokeColor=None))

                # Label segments only when there is enough room. The exact-value
                # table below every report chart lists all values, including small segments.
                if not math.isclose(value, 0.0) and segment_height >= 15:
                    drawing.add(String(
                        x + bar_width / 2,
                        min(y1, y2) + segment_height / 2 - 2.2,
        ...
        for category_index in range(len(category_names)):
            ...
            for series_index, (_, values, hex_color) in enumerate(series):
                ...
                # Place the exact value at the end of each bar so the PDF remains
                # readable without relying on interactive hover information.
                if value >= 0:
                    label_y = value_y + 3.5
                else:
                    label_y = value_y - 9.0
...
def report_chart_values_table(
    ...
    *,
    ...
    """Create a compact exact-value table to accompany a PDF chart."""
    available_width = 7.8 * inch
    first_col_width = 1.55 * inch
    value_col_count = max(1, len(series))
    value_col_width = (available_width - first_col_width) / value_col_count
    ...
    data: list[list[Any]] = [[
        ...
        *[
            Paragraph(
                pdf_safe_text(f"{label}{' (' + unit_label + ')' if unit_label else ''}"),
                header_style,
            )
    ...
    for category_index, category_name in enumerate(category_names):
        row: list[Any] = [Paragraph(pdf_safe_text(category_name), assembly_style)]
        for _, values, _ in series:
            value = float(values[category_index])
            row.append(Paragraph(f"{value:,.{value_decimals}f}", cell_style))
        data.append(row)

    table = Table(
        data,
        repeatRows=1,
        colWidths=[first_col_width] + [value_col_width] * value_col_count,
        hAlign="CENTER",
    )
...
def comparison_report_bytes() -> bytes:
    ...
    if started.empty:
        ...
        intensity_series = [
            ("Emitted", started["Emitted kgCO2e/m2"].astype(float).tolist(), COLORS["purple"]),
            ("Net", started["Net kgCO2e/m2"].astype(float).tolist(), COLORS["blue"]),
        ]
        total_series = [
            ("Emitted", started["Emitted kgCO2e"].astype(float).tolist(), COLORS["purple"]),
            ("Stored", started["Stored kgCO2e"].astype(float).tolist(), COLORS["green"]),
            ("Net", started["Net kgCO2e"].astype(float).tolist(), COLORS["blue"]),
        ]

        story.extend([
            PageBreak(),
            Paragraph("Intensity comparison", chart_heading_style),
            report_bar_chart(category_names, intensity_series, "kgCO2e/m2", value_decimals=3),
            Spacer(1, 5),
            Paragraph("Exact values", body_style),
            Spacer(1, 3),
            report_chart_values_table(category_names, intensity_series, value_decimals=3, unit_label="kgCO2e/m2"),
            PageBreak(),
            Paragraph("Total carbon", chart_heading_style),
            report_bar_chart(category_names, total_series, "kgCO2e", value_decimals=2),
            Spacer(1, 5),
            Paragraph("Exact values", body_style),
            Spacer(1, 3),
            report_chart_values_table(category_names, total_series, value_decimals=2, unit_label="kgCO2e"),
        ])
        ...
        if detail_rows:
            ...
            if category_series:
                story.extend([
                    ...
                    report_bar_chart(category_names, category_series, "Emitted kgCO2e", stacked=True, value_decimals=2),
                    Spacer(1, 5),
                    Paragraph("Exact values by category", body_style),
                    Spacer(1, 3),
                    report_chart_values_table(category_names, category_series, value_decimals=2, unit_label="kgCO2e"),
                    Spacer(1, 6),
                    Paragraph("Each bar shows emitted A1-A3 carbon, stacked by material category. Exact material-level values are also included in the assembly detail pages.", body_style),
                ])
    ...
    for assembly, result in assembly_results:
        ...
        # Use Paragraph objects in every detail-table cell. ReportLab does not
        # wrap ordinary strings inside fixed-width table cells, which causes long
        # material names, categories, units, and formulas to draw over adjacent
        # columns. Paragraphs calculate their own wrapped height so each row grows
        # as needed and remains readable.
        ...
        material_text_style = ParagraphStyle(
            "MaterialTableText",
            fontName="Helvetica",
            fontSize=6.2,
            leading=7.5,
        ...
        material_number_style = ParagraphStyle(
            "MaterialTableNumber",
            parent=material_text_style,
            alignment=TA_RIGHT,
        )

        def material_cell(value: Any, style: ParagraphStyle = material_text_style) -> Paragraph:
            # Escape ReportLab paragraph markup characters while retaining the
            # app's safe ASCII substitutions for PDF font compatibility.
            safe = pdf_safe_text(value).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        ...
        material_data: list[list[Any]] = [[
            material_cell(label, material_header_style) for label in material_headers
        ]]
        for row in result["rows"]:
            if row["status"] == "Not selected":
                continue
            quantity_text = (
                f'{row["quantity"]:,.3f} {pdf_safe_text(row["quantity_unit"])}'
                if row["valid"] else "-"
            )
            formula_text = row["formula_note"] if row["valid"] else row["error"] or row["status"]
            material_data.append([
                material_cell(row["category"]),
                material_cell(row["name"]),
                material_cell(row["unit"]),
                material_cell(quantity_text, material_number_style),
                material_cell(f'{row["emitted"]:,.2f}' if row["valid"] else "-", material_number_style),
                material_cell(f'{row["stored"]:,.2f}' if row["valid"] else "-", material_number_style),
                material_cell(f'{row["net"]:,.2f}' if row["valid"] else "-", material_number_style),
                material_cell(formula_text),
            ])
        ...
        if len(material_data) == 1:
            ...
            material_table = Table(
                material_data,
                repeatRows=1,
                # Use almost the full 10.1-inch printable width. The two text-heavy
                # columns receive most of the added space while numeric columns stay compact.
                colWidths=[
                    1.15 * inch,
                    2.75 * inch,
                    0.85 * inch,
    ...
    if started.empty:
        ...
        with tab1:
            intensity_long = started.melt(
                id_vars=["Assembly"],
                value_vars=["Emitted kgCO₂e/m²", "Net kgCO₂e/m²"],
            ...
            fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", legend_title_text="", xaxis_title="", margin=dict(t=42))
            fig.update_yaxes(gridcolor=COLORS["line"])
            st.plotly_chart(fig, width="stretch")
            st.caption("Exact values (kgCO₂e/m²)")
            intensity_values = started[["Assembly", "Emitted kgCO₂e/m²", "Net kgCO₂e/m²"]].copy()
            st.dataframe(
                intensity_values.style.format({"Emitted kgCO₂e/m²": "{:.3f}", "Net kgCO₂e/m²": "{:.3f}"}),
                width="stretch",
                hide_index=True,
            )
        ...
        with tab2:
            ...
            st.plotly_chart(fig, width="stretch")
            st.caption("Exact values (kgCO₂e)")
            total_values = started[["Assembly", "Emitted kgCO₂e", "Stored kgCO₂e", "Net kgCO₂e"]].copy()
            st.dataframe(
                total_values.style.format({"Emitted kgCO₂e": "{:.2f}", "Stored kgCO₂e": "{:.2f}", "Net kgCO₂e": "{:.2f}"}),
                width="stretch",
                hide_index=True,
            )
        ...
        with tab3:
            if detail_rows:
                ...
                fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", legend_title_text="", xaxis_title="", margin=dict(t=20), uniformtext_minsize=8, uniformtext_mode="hide")
                fig.update_yaxes(gridcolor=COLORS["line"])
                st.plotly_chart(fig, width="stretch")
                st.caption("Exact emitted A1-A3 values by category (kgCO₂e)")
                breakdown_values = by_category.pivot(index="Assembly", columns="Category", values="Emitted kgCO₂e").fillna(0.0).reset_index()
                ordered_breakdown_columns = ["Assembly"] + [category for category in STARTING_CATEGORIES if category in breakdown_values.columns]
                breakdown_values = breakdown_values[ordered_breakdown_columns]
                breakdown_formats = {column: "{:.2f}" for column in breakdown_values.columns if column != "Assembly"}
                st.dataframe(breakdown_values.style.format(breakdown_formats), width="stretch", hide_index=True)
