...
def pdf_paragraph(value: Any, style: ParagraphStyle) -> Paragraph:
    """Create an escaped, wrapping paragraph for PDF table cells."""
    safe = escape(pdf_safe_text(value)).replace("\n", "<br/>")
...
def comparison_report_bytes() -> bytes:
    ...
    table_formula_style = ParagraphStyle(
        "ReportTableFormula",
        parent=table_text_style,
        fontSize=6.0,
        leading=7.3,
    ...
    if started.empty:
        ...
        summary_data: list[list[Any]] = [
            [pdf_paragraph(value, table_header_style) for value in summary_header]
        ]
        for _, row in comparison.iterrows():
            summary_data.append([
                pdf_paragraph(row["Assembly"], table_text_style),
                pdf_paragraph(f'{row["Width (ft)"]:,.1f} ft', table_number_style),
                pdf_paragraph(f'{row["Height (ft)"]:,.1f} ft', table_number_style),
                pdf_paragraph(f'{row["Area (ft2)"]:,.1f} ft2', table_number_style),
                pdf_paragraph(row["Materials ready"], table_number_style),
                pdf_paragraph(f'{row["Emitted kgCO2e"]:,.2f}', table_number_style),
                pdf_paragraph(f'{row["Stored kgCO2e"]:,.2f}', table_number_style),
                pdf_paragraph(f'{row["Net kgCO2e"]:,.2f}', table_number_style),
                pdf_paragraph(f'{row["Emitted kgCO2e/m2"]:,.3f}', table_number_style),
                pdf_paragraph(f'{row["Net kgCO2e/m2"]:,.3f}', table_number_style),
                pdf_paragraph(row["Status"], table_text_style),
            ])

        summary_table = Table(
            summary_data,
            repeatRows=1,
            colWidths=[1.55 * inch, 0.60 * inch, 0.60 * inch, 0.72 * inch, 0.55 * inch, 0.75 * inch, 0.75 * inch, 0.75 * inch, 0.88 * inch, 0.82 * inch, 0.78 * inch],
            hAlign="CENTER",
        )
    ...
    for assembly, result in assembly_results:
        ...
        material_data: list[list[Any]] = [
            [pdf_paragraph(value, table_header_style) for value in material_headers]
        ]
        for row in result["rows"]:
            if row["status"] == "Not selected":
                continue
            material_data.append([
                pdf_paragraph(row["category"], table_text_style),
                pdf_paragraph(row["name"], table_text_style),
                pdf_paragraph(row["unit"], table_text_style),
                pdf_paragraph(
                    f'{row["quantity"]:,.3f} {pdf_safe_text(row["quantity_unit"])}' if row["valid"] else "-",
                    table_number_style,
                ),
                pdf_paragraph(f'{row["emitted"]:,.2f}' if row["valid"] else "-", table_number_style),
                pdf_paragraph(f'{row["stored"]:,.2f}' if row["valid"] else "-", table_number_style),
                pdf_paragraph(f'{row["net"]:,.2f}' if row["valid"] else "-", table_number_style),
                pdf_paragraph(
                    row["formula_note"] if row["valid"] else row["error"] or row["status"],
                    table_formula_style,
                ),
            ])
        ...
        if len(material_data) == 1:
            ...
            material_table = Table(
                material_data,
                repeatRows=1,
                colWidths=[1.25 * inch, 2.65 * inch, 0.95 * inch, 1.10 * inch, 0.65 * inch, 0.65 * inch, 0.65 * inch, 2.10 * inch],
                hAlign="CENTER",
            )
...
# =============================================================================
    ...
    if started.empty:
        ...
        with tab1:
            ...
            fig = px.bar(intensity_long, x="Assembly", y="kgCO₂e/m²", color="Measure", barmode="group", color_discrete_sequence=[COLORS["purple"], COLORS["blue"]])
