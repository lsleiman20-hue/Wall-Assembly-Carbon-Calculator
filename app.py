...
CATEGORY_COLORS = {
    "Exterior Cladding": "#C15A3C",
    "Attachment System": "#E49B32",
    "Insulation": "#708150",
    "Stud and Backup Framing": "#355972",
    "Moisture and Air Control": "#6A8CA3",
    "Sheathing": "#91765A",
    "Masonry and Concrete": "#8A8178",
    "Reinforcing and Ties": "#59636B",
    "Finish": "#A4778B",
    "Other": "#7A8790",
}
...
st.markdown(
    ...
    <style>
      ...
      [data-testid="stSidebar"] { background: #234058; }
      [data-testid="stSidebar"] * { color: #F8FAFB; }
      [data-testid="stSidebar"] input { color: #24313A; }
      [data-testid="stSidebar"] div[data-baseweb="select"] * { color: #24313A; }
      [data-testid="stMetric"] {
        background: #FFFFFF;
        border: 1px solid #DDE2E4;
        border-radius: 16px;
        padding: 14px 16px;
        box-shadow: 0 2px 10px rgba(35, 64, 88, 0.05);
      }
      [data-testid="stMetricLabel"] { color: #66737D; }
      [data-testid="stMetricValue"] { color: #234058; }
      .hero {
        background: linear-gradient(135deg, #234058 0%, #355972 72%, #708150 150%);
        border-radius: 22px;
        padding: 28px 30px;
        margin-bottom: 20px;
        color: white;
        box-shadow: 0 10px 30px rgba(35, 64, 88, 0.18);
      }
      .hero h1 { font-size: 2.2rem; margin: 0; letter-spacing: -0.02em; }
      .hero p { margin: 8px 0 0 0; opacity: 0.86; max-width: 820px; }
      .section-label {
        color: #234058;
        font-size: 0.78rem;
        font-weight: 800;
      ...
      .epd-card {
        background: #FFFFFF;
        border: 1px solid #DDE2E4;
        border-left: 5px solid #E49B32;
        border-radius: 14px;
        padding: 13px 15px;
        margin: 8px 0 12px 0;
      }
      .epd-card strong { color: #234058; }
      .small-muted { color: #66737D; font-size: 0.88rem; }
      .formula-box {
        background: #EEF2F3;
        border-radius: 12px;
        padding: 12px 14px;
      ...
      .status-ready { color: #53643C; font-weight: 700; }
      .status-check { color: #A34A35; font-weight: 700; }
...
PRESET_MATERIALS = ast.literal_eval(r"""[
  {
...
# Size-specific factors supplied for SPF lumber. Values are per installed metre.
# Stored/biogenic carbon is negative in calculator results because it offsets
# emitted carbon when net carbon is calculated.
WOOD_SIZE_FACTORS = {
    "1x2": {"emitted": 0.046, "stored": -0.5514},
    "1x3": {"emitted": 0.076, "stored": -0.9190},
    "1x4": {"emitted": 0.107, "stored": -1.2866},
    "1x6": {"emitted": 0.168, "stored": -2.0218},
    "1x8": {"emitted": 0.221, "stored": -2.6650},
    "1x10": {"emitted": 0.283, "stored": -3.4002},
    "2x3": {"emitted": 4.008, "stored": -48.24},
    "2x4": {"emitted": 5.611, "stored": -67.54},
    "2x6": {"emitted": 8.818, "stored": -106.13},
    "2x8": {"emitted": 11.624, "stored": -139.90},
    "2x10": {"emitted": 14.830, "stored": -178.49},
    "2x12": {"emitted": 18.037, "stored": -217.08},
}
...
def category_list() -> list[str]:
    preferred = list(CATEGORY_COLORS)
    found_set = {item["category"] for item in all_library_materials()}
    found_set.add("Other")
    found = sorted(found_set)
    return [x for x in preferred if x in found] + [x for x in found if x not in preferred]
...
def effective_epd(material: dict[str, Any]) -> dict[str, Any] | None:
    if material.get("preset") == "Other (custom EPD)":
        return {
...
def calculate_material(assembly: dict[str, Any], material: dict[str, Any]) -> dict[str, Any]:
    ...
    base = {
        "valid": False,
        "status": "Excluded" if material.get("use") != "Yes" else "Check inputs",
        "error": "",
        "quantity": 0.0,
        "quantity_unit": "",
        "emitted": 0.0,
    ...
    if epd is None:
        ...
        base["error"] = "Select a preset material or choose Other (custom EPD)."
        return base
    if material.get("preset") == "Other (custom EPD)" and not material.get("custom_name", "").strip():
    ...
    try:
        if method == "Area":
            ...
            if stud_size not in WOOD_SIZE_FACTORS:
            ...
            quantity = count * height_ft * FT_TO_M
            quantity_unit = "m"
            size_factors = WOOD_SIZE_FACTORS[stud_size]
            epd = {
                **epd,
                "emitted": size_factors["emitted"],
                "stored": size_factors["stored"],
                "unit": "kgCO₂e/m",
            }
            note = (
                f'{count} studs × {height_ft:g} ft × 0.3048 = {quantity:,.3f} m; '
                f'{stud_size} factors: {size_factors["emitted"]:g} emitted and '
                f'{size_factors["stored"]:g} stored kgCO₂e/m'
            )
...
def calculate_assembly(assembly: dict[str, Any]) -> dict[str, Any]:
    ...
    included = [row for row in rows if row["status"] != "Excluded"]
...
def render_method_inputs(material: dict[str, Any], method: str, prefix: str) -> None:
    ...
    if method == "Area":
        ...
        c1, c2 = st.columns(2)
        with c1:
            number_field(material, "grammage_kg_m2", "Grammage (kg/m²)", f"{prefix}_grammage", 1.0, 0.1)
        with c2:
            dropdown_field(material, "adjust_spacing", "Adjust grammage for spacing?", ["No", "Yes"], f"{prefix}_adjust_spacing", "No")
        if input_value(material, "adjust_spacing", "No") == "Yes":
            c1, c2 = st.columns(2)
            with c1:
                number_field(material, "reference_spacing_in", "Reference spacing (in)", f"{prefix}_ref_spacing", 16.0, 1.0)
            with c2:
                number_field(material, "actual_spacing_in", "Actual spacing (in)", f"{prefix}_actual_spacing", 16.0, 1.0)
            st.caption("Mass is multiplied by reference spacing ÷ actual spacing. Closer spacing increases mass.")
        ...
        c1, c2, c3 = st.columns(3)
        with c1:
            dropdown_field(material, "stud_size", "Stud size", list(WOOD_SIZE_FACTORS), f"{prefix}_stud_size", "2x4")
        with c2:
...
def style_header(ws, row: int, start_col: int, end_col: int) -> None:
    fill = PatternFill("solid", fgColor="234058")
...
def workbook_bytes() -> bytes:
    ...
    headers = [
        ...
        "Emitted kgCO₂e/ft²", "Net kgCO₂e/ft²", "Status", "Notes",
    ]
    ...
    for assembly in st.session_state.assemblies:
        ...
        summary.append([
            ...
            result["net"], result["emitted_intensity"], result["net_intensity"], result["status"], assembly["notes"],
        ])

        ws = wb.create_sheet(safe_sheet_name(assembly["name"], used_names))
        ws["A1"] = "Wall Assembly Embodied Carbon"
        ws["A1"].font = Font(size=16, bold=True, color="234058")
        ...
        ws["A8"] = "Notes"
        ws["B8"] = assembly["notes"]

        material_headers = [
            "Use", "Category", "Material", "Method", "Declared Unit", "Declared Quantity", "Quantity Unit",
        ...
        for material, row in zip(assembly["materials"], result["rows"]):
            ws.append([
                material.get("use", "Yes"), row["category"], row["name"], row["method"], row["unit"],
...
with st.sidebar:
    st.markdown("## 🌿 Wall Carbon")
    st.caption("EPD-based wall assembly calculator")
    ...
    if uploaded is not None and st.button("Load uploaded project", width="stretch"):
        try:
            ...
            st.session_state.custom_library = payload.get("custom_library", [])
            st.session_state.active_assembly_id = assemblies[0]["id"]
...
if page == "Build Assemblies":
    st.markdown(
        """
        <div class="hero">
          <h1>Wall Assembly Carbon Calculator</h1>
          <p>Build named wall options from verified preset EPDs or custom EPD values. Inputs change automatically based on the declared unit and calculation method.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    top1, top2, top3, top4 = st.columns([4.5, 1.3, 1.3, 1.3])
    with top1:
        selected_label = st.selectbox("Wall option", assembly_labels, index=active_index)
        selected_index = assembly_labels.index(selected_label)
        selected_id = st.session_state.assemblies[selected_index]["id"]
    ...
    with top3:
        ...
        if st.button("⧉ Duplicate", width="stretch"):
            ...
            st.session_state.assemblies.append(duplicated)
    ...
    with top4:
        ...
        if st.button("Delete", width="stretch", disabled=len(st.session_state.assemblies) == 1):
            st.session_state.assemblies = [a for a in st.session_state.assemblies if a["id"] != active["id"]]
            st.session_state.active_assembly_id = st.session_state.assemblies[0]["id"]
            st.rerun()

    active = get_active_assembly()
    with st.container(border=True):
        st.markdown('<div class="section-label">Wall option information</div>', unsafe_allow_html=True)
        ...
        active["notes"] = st.text_area("Notes / key differences", value=active.get("notes", ""), key=f'notes_{active["id"]}', height=76)
    ...
    with right:
        if st.button("＋ Add material", width="stretch"):
            active["materials"].append(new_material())
            st.rerun()

    if not active["materials"]:
        st.info("Add the first material to begin this wall option.")

    delete_id = None
    for position, material in enumerate(active["materials"], start=1):
        existing_result = calculate_material(active, material)
        title_name = existing_result["name"] if material.get("preset") else "Select a material"
        status_icon = "●" if existing_result["valid"] else ("○" if existing_result["status"] == "Excluded" else "!")
        with st.expander(f'{status_icon}  Material {position} — {title_name}', expanded=not existing_result["valid"]):
            prefix = f'{active["id"]}_{material["id"]}'
            h1, h2 = st.columns([1, 0.25])
            with h1:
                material["use"] = st.selectbox("Include this material?", ["Yes", "No"], index=select_index(["Yes", "No"], material.get("use", "Yes")), key=f'{prefix}_use')
            with h2:
                st.write("")
                if st.button("Remove", key=f'{prefix}_remove', width="stretch"):
                    delete_id = material["id"]

            if material["use"] == "No":
                st.caption("This row is excluded from all totals.")
                continue

            c1, c2 = st.columns([1, 2.2])
            with c1:
                categories = category_list()
                material["category"] = st.selectbox("Material type", categories, index=select_index(categories, material.get("category")), key=f'{prefix}_category')
            with c2:
                choices = presets_for_category(material["category"])
                current_preset = material.get("preset", "") if material.get("preset", "") in choices else ""
                material["preset"] = st.selectbox("Preset material / EPD", choices, index=select_index(choices, current_preset), key=f'{prefix}_preset_{material["category"]}')

            if not material["preset"]:
                st.warning("Select a preset or choose Other (custom EPD).")
                continue

            if material["preset"] == "Other (custom EPD)":
                st.markdown('<div class="section-label">Custom EPD</div>', unsafe_allow_html=True)
                material["custom_name"] = st.text_input("Custom material name", value=material.get("custom_name", ""), key=f'{prefix}_custom_name')
                c1, c2 = st.columns(2)
                with c1:
                ...
                with c2:
                    material["custom_stored"] = st.number_input("Stored carbon per declared unit", value=float(material.get("custom_stored", 0.0)), step=0.1, key=f'{prefix}_custom_stored', help="Use a negative value for carbon stored or sequestered.")
                c1, c2 = st.columns(2)
                with c1:
                ...
                epd = effective_epd(material)
                if epd:
                    verification = "Verified" if "Verified" in epd.get("source", "") else "Preset"
                    if epd["method"] == "Wood Stud Framing":
                        factor_text = "Emitted and stored factors update automatically with the selected lumber size."
                    else:
                        factor_text = (
                            f'Emitted: {epd["emitted"]:,.6g} {epd["unit"]} &nbsp;•&nbsp; '
                            f'Stored: {epd["stored"]:,.6g} {epd["unit"]}'
                        )
                    st.markdown(
                        f"""<div class="epd-card"><strong>{verification} EPD preset</strong><br>
                        <span class="small-muted">{factor_text}<br>
                        Method: {epd["method"]} &nbsp;•&nbsp; {epd.get("source", "")}</span></div>""",
                        unsafe_allow_html=True,
                    )
            ...
            if epd:
                ...
                st.markdown('<div class="section-label">Calculated result</div>', unsafe_allow_html=True)
                if row["valid"]:
                    r1, r2, r3, r4 = st.columns(4)
                    r1.metric("Declared quantity", f'{row["quantity"]:,.3f} {row["quantity_unit"]}')
    ...
    if delete_id:
        active["materials"] = [m for m in active["materials"] if m["id"] != delete_id]
        st.rerun()
    ...
    status_placeholder.caption(f'{result["ready_count"]} of {result["selected_count"]} selected materials are ready.')
    ...
    if result["rows"]:
        ...
        if ready_rows:
            ...
            fig = px.bar(
                ...
                height=max(360, 44 * len(chart_df)),
            )
            fig.update_layout(
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                legend_title_text="", margin=dict(l=10, r=10, t=20, b=10),
                xaxis_title="kgCO₂e", yaxis_title="",
            )
            fig.update_xaxes(gridcolor="#DDE2E4")
...
# =============================================================================
    ...
    if started.empty:
        ...
        with tab1:
            ...
            fig = px.bar(intensity_long, x="Assembly", y="kgCO₂e/ft²", color="Measure", barmode="group", color_discrete_sequence=[COLORS["amber"], COLORS["blue"]])
            fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", legend_title_text="", xaxis_title="", margin=dict(t=20))
            fig.update_yaxes(gridcolor="#DDE2E4")
            st.plotly_chart(fig, width="stretch")
        ...
        with tab2:
            ...
            fig = px.bar(total_long, x="Assembly", y="kgCO₂e", color="Measure", barmode="group", color_discrete_sequence=[COLORS["amber"], COLORS["green"], COLORS["blue"]])
            fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", legend_title_text="", xaxis_title="", margin=dict(t=20))
            fig.update_yaxes(gridcolor="#DDE2E4", zerolinecolor="#9AA5AC")
            st.plotly_chart(fig, width="stretch")
...
# =============================================================================
    ...
    guides = [
        ...
        ("Mass by Grammage", "kgCO₂e/kg", "Mass = wall area in m² × grammage in kg/m² × reference spacing ÷ actual spacing (spacing adjustment optional)"),
